...
// GET /api/admin/users - List all registered users and what products they bought
router.get("/users", async (req: Request, res: Response) => {
  try {
    ...
    // Fallback response if DB has no users or connection issue
    res.json([
      {
        id: 1,
        name: "Rahul Sharma",
...
// POST /api/admin/users - Admin creates new user account in PostgreSQL
router.post("/users", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Dispatch Welcome Email containing Password Change / Reset Link to user's email address
...
// GET /api/admin/stats - Admin dashboard metrics & product stock count
...
// DELETE /api/admin/users/:email - Admin deletes user permanently from PostgreSQL
  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        // Delete user permanently from PostgreSQL
